...
const CardActionsStep = React.memo(
    ({ cardId, defaultStep, onNameEdit, onClose }) => {
        const selectCardById = useMemo(
            () => selectors.makeSelectCardById(),
            [],
        );
        const selectListById = useMemo(
            () => selectors.makeSelectListById(),
            [],
        );
        const selectPrevListById = useMemo(
            () => selectors.makeSelectListById(),
            [],
        );
        ...
        const prevList = useSelector(
            (state) =>
                card.prevListId && selectPrevListById(state, card.prevListId),
        );

        const userIds = useSelector((state) =>
            selectUserIdsByCardId(state, cardId),
        );
        ...
        const withActionBar = useMemo(() => {
            ...
            if (card.type === CardTypes.PROJECT && canUseMembers)
                menuItemsTotal += 1;
            if (canUseLabels) menuItemsTotal += 1;
            if (card.type !== CardTypes.PROJECT && canUseMembers)
                menuItemsTotal += 1;
            if (card.type === CardTypes.PROJECT && canEditDueDate)
                menuItemsTotal += 1;
            if (card.type === CardTypes.PROJECT && canEditStopwatch)
                menuItemsTotal += 1;
            if (canEditName) menuItemsTotal += 1;
            if (!board.limitCardTypesToDefaultOne && canEditType)
                menuItemsTotal += 1;
        ...
        const handleRestoreClick = useCallback(() => {
            dispatch(
                entryActions.moveCard(cardId, card.prevListId, undefined, true),
            );
        ...
        const handleEditRecurrenceClick = useCallback(() => {
            openStep(StepTypes.EDIT_RECURRENCE);
        }, [openStep]);
        ...
        if (step) {
            switch (step.type) {
                ...
                case StepTypes.EDIT_RECURRENCE:
                    return (
                        <EditRecurrenceStep
                            cardId={cardId}
                            onBack={handleBack}
