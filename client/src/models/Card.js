    ...
    static reducer({ type, payload }, Card) {
        switch (type) {
            ...
            case ActionTypes.CARD_UPDATE: {
                ...
                if (
                    payload.data.listId &&
                    payload.data.listId !== cardModel.listId
                ) {
                ...
                if (
                    payload.data.isClosed !== undefined &&
                    payload.data.isClosed !== cardModel.isClosed
                ) {
            ...
            case ActionTypes.CARD_UPDATE_HANDLE: {
                ...
                if (payload.card.boardId !== null) {
                    ...
                    if (
                        cardModel &&
                        payload.card.isClosed !== cardModel.isClosed
                    ) {
                        cardModel.linkedTasks.update({
            ...
            case ActionTypes.COMMENTS_FETCH__SUCCESS:
                Card.withId(payload.cardId).update({
                    ...
                    isAllCommentsFetched:
                        payload.comments.length < Config.COMMENTS_LIMIT,
                    ...(payload.comments.length > 0 && {
                        lastCommentId:
                            payload.comments[payload.comments.length - 1].id,
                    }),
                });
    ...
    syncAfterBoardChange() {
        ...
        this.taskLists.toModelArray().forEach((taskListModel) => {
            taskListModel.tasks.toModelArray().forEach((taskModel) => {
                if (
                    !taskModel.assigneeUserId ||
                    userIdsSet.has(taskModel.assigneeUserId)
                ) {
                    return;
                }
